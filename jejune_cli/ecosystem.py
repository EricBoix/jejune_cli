...
def discover_doc_repos(
    ...
    """Return all locally cloned jejune_doc_* repos with a flag for doc.yaml presence."""
    seen: set[str] = set()
    ...
    for tier, base in (("root", root_dir), ("tmp", tmp_dir)):
        ...
        for p in sorted(base.glob("jejune_doc_*")):
            if p.is_dir() and p.name not in seen:
                seen.add(p.name)
                results.append((p.name, tier, str(p), (p / "doc.yaml").exists()))  # type: ignore[arg-type]
    return results
...
def ecosystem_status() -> None:
    ...
    click.echo(f"  {'Repository':<{_W_DN}}  doc.yaml  Clone directory")
    click.echo("  " + "─" * (_W_DN + 2 + len("doc.yaml") + 2 + _W_DC))
